r"""Implementation of SAC+AE: https://arxiv.org/abs/1910.01741"""
...
def train(encoder_network_def,
  ...
  rae_grad_fn = jax.value_and_grad(rae_loss_fn)
  rae_loss, rae_gradient = rae_loss_fn(encoder_params,
                                             dynamics_params)
...
#   reward_gradient, dynamics_gradient = dynamics_gradients
  ...
  encoder_updates, encoder_optim_state = encoder_optim.update(
      rae_gradient, encoder_optim_state, params=encoder_params)
...
@gin.configurable
class SACAEAgent(sac_agent.SACAgent):
